...
/// 6. Optionally dumps PTX if `PYROS_DUMP_PTX` is set
pub fn generate_build_ptx(sig: &KernelSignature, body: &[KernelStmt]) -> syn::Result<TokenStream> {
    ...
    Ok(quote! {
        fn build_ptx() -> String {
            ...
            use pyros::core::instr::ArithOp;
            use pyros::core::instr::control::{CmpOp, ControlOp};
            use pyros::core::instr::memory::MemoryOp;
            use pyros::core::instr::special;
            use pyros::core::ir::{
