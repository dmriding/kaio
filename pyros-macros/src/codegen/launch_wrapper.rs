...
pub fn generate_launch_fn(sig: &KernelSignature) -> syn::Result<TokenStream> {
    ...
    launch_params.push(quote! { device: &pyros::runtime::PyrosDevice });
    ...
    for param in &sig.params {
        ...
        match &param.ty {
            KernelType::SliceRef(elem) => {
                ...
                launch_params.push(quote! { #name: &pyros::runtime::GpuBuffer<#rust_ty> });
                arg_calls.push(quote! { .arg(#name.inner()) });
            }
            KernelType::SliceMutRef(elem) => {
                let rust_ty = rust_type_tokens(elem);
                launch_params.push(quote! { #name: &mut pyros::runtime::GpuBuffer<#rust_ty> });
    ...
    Ok(quote! {
        ...
        pub fn launch(#(#launch_params),*) -> Result<(), pyros::runtime::PyrosError> {
            use pyros::runtime::PushKernelArg;

            let ptx = PTX_CACHE.get_or_init(build_ptx);
            let module = device.load_ptx(ptx)?;
            let func = module.function(#kernel_name)?;
            let cfg = pyros::runtime::LaunchConfig::for_num_elems(#n_ident);
