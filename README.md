...
| Tensor-core matmul (bf16)                | `kaio_ops::matmul_tc_bf16` — bf16 × bf16 → f32, SM 8.0+, `K % 16 == 0`. Sync sibling of `matmul_tc` using `mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32`. Perf parity with f16 sync at 4096³ (Phase 9 aggregate release will publish the table). |
