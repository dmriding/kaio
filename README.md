...
| Tensor-core matmul (bf16)                | `kaio_ops::matmul_tc_bf16` / `matmul_tc_bf16_async` — bf16 × bf16 → f32, SM 8.0+, `K % 16 == 0`. Sync and cp.async-pipelined siblings of `matmul_tc` / `matmul_tc_async` using `mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32`. Perf parity with the f16 path at 4096³ on RTX 4090 sm_89 — async SC-2 median ratio +0.72% / worst +1.55% within the ±3% / ±15% split-bound gate (Phase 9 aggregate release will publish the full table). |
