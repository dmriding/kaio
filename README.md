...
| Fused tri-output QKV projection (INT8, W8A16) | `kaio_ops::qkv_project_int8` — f16 activations × i8 weights × scalar per-projection scales → three f16 outputs ready for `attention_tc`. Single kernel launch saves 2× global activation reads vs 3× `matmul_int8`; **~3.0× faster than three standalone calls at decode shapes (M ≤ 128), 1.19× at mid prefill (M=512), 0.85× at large prefill (M=2048)**. Call three separate projections for prefill-heavy workloads. SM 8.0+, K%16==0, N%2==0. Sprint 7.3 on `phase7-rest`. |
| Fused tri-output QKV projection (INT4, W4A16) | `kaio_ops::qkv_project_int4` — packed signed-INT4 weights × f16 activations × f16 group scales → three f16 outputs ready for `attention_tc`. Same packing convention as `matmul_int4`. Same decode/prefill profile as `qkv_project_int8`. SM 8.0+, K%128==0, N%2==0, group_size=128 fixed. Sprint 7.3 on `phase7-rest`. |
