...
- `ArithOp` instructions: `Add`, `Mad` (with `MadMode::Lo`), `MulWide` — the minimum set for `vector_add`, validated byte-for-byte against nvcc 12.8 PTX output
- Instruction category stubs: `MemoryOp`, `ControlOp` (uninhabited — populated in upcoming sprints)
